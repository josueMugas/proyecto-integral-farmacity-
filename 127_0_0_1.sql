...
--
-- Servidor: 127.0.0.1
-- Tiempo de generación: 20-11-2025 a las 12:50:18
...
CREATE TABLE `medicamentos` (
  ...
  `codigo_barras` varchar(200) DEFAULT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
...
INSERT INTO `ventas` (`id`, `clientes_id`, `medicamentos_id`, `fecha`, `cantidad`, `empleado_id`) VALUES
(1, 1, 2, '0000-00-00', 12, ''),
(2, 3, 1, '0000-00-00', 11, ''),
(3, 6, 4, '2025-11-01', 11, ''),
(4, 6, 5, '2025-10-01', 9, ''),
(5, 2, 3, '0000-00-00', 8, ''),
(6, 6, 6, '2025-11-09', 14, ''),
(7, 6, 5, '2025-10-26', 109, ''),
(8, 6, 5, '2025-11-25', 102, '');

--
-- Disparadores `ventas`
--
DROP TRIGGER IF EXISTS `cantidades`;
DELIMITER $$
CREATE TRIGGER `cantidades` BEFORE UPDATE ON `ventas` FOR EACH ROW BEGIN
    UPDATE clientes
    SET cantidad_compras = (
        SELECT COUNT(Clientes_id)
        FROM ventas
        WHERE clientes_id = OLD.clientes_id
    )
    WHERE id = OLD.clientes_id;
END
$$
DELIMITER ;
...
ALTER TABLE `ventas`
  ...
  ADD KEY `empleado_id` (`empleado_id`(191));
